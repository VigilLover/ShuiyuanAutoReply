...
- **Web channel** (`interfaces/api/app.py:create_app` + `bootstrap/container.py:ApplicationContainer.for_api`): uses `LazyForum`/`LazyChat` so the UI starts even without a forum login or API key. Handlers are help, `【rua】`, chat. The service is wrapped in `_SwappableBotService` so the web runtime can be hot-swapped from the settings page. `app.py` only assembles the `FastAPI` instance and its middleware; routes are `APIRouter`s under `interfaces/api/routes/{compat,conversations,forum_monitor,settings_profiles,model_configs,tools,artifacts,static}.py`, and shared request-scoped helpers (`SessionRegistry`, `state_store`, `profile_defaults`) live in `interfaces/api/support.py`.

Boundary rules (enforced by `test/test_architecture_boundaries.py`): `application/` never imports `infrastructure/`, aiohttp, fastapi, sqlalchemy, neo4j; `domain/` is stdlib-only; `features/` never imports `interfaces/` (interfaces wire features together, not the reverse). `application/ports/` holds the Protocols infrastructure implements; extend `test/test_port_contracts.py` when adding or changing one.
...
`MentionChatModel` itself keeps `__init__`, `initialize_agent`, `_build_graph`, `_call_model`, and `get_pumpkin_response`; the rest of the node bodies live in three mixins it composes: `features/mention/context.py:ContextMixin` (style/topic/memory/image context loading, `recent_msgs`), `features/mention/tools_runtime.py:ToolsRuntimeMixin` (MCP tool adapters, tool-call validation and execution), and `features/mention/finalize.py:FinalizeMixin` (finalizer prompt, response parsing, history save). `MentionGraphState` lives in `features/mention/graph_state.py`. `features/mention/chat_pipeline.py:ChatOrchestrator` still wraps each node as a distinct callable (needed for LangGraph node identity and, for the multimodal loaders, an `image.timing` event) but delegates straight back to `self.owner._method(state)` on the composed instance — it holds no node logic itself. A method called across mixin boundaries without a bound `self`/`cls` (e.g. from a `@staticmethod`) must go through `self.`/`cls.`, not the old bare class name, or it reintroduces a circular import. `features/mention/context_budget.py` does token-budget compaction and **tool call/result pairing repair**; breaking pairing yields a 400 from OpenAI-compatible Responses endpoints, so run `test_tool_call_pairing.py` and `test_convergence_regressions.py` after touching the loop. Forum/web/user tools are exposed through `features/mention/shuiyuan_tools_wrapper.py` and `tool_catalog.py` (one tool per capability: `forum_search`, `forum_read`, `users`, `web_search`, `web_read`; legacy names are remapped by `migrate_tool_names`); `web_*` come from the external SimpleMCP server via `langchain-mcp-adapters`. Details and limits: `docs/agent-tools.md`.

Tests that patch `emit_event` around one of these node methods must patch it at the module that now defines the method (`mention_chat_model`, `context`, `tools_runtime`, or `finalize`), not always `mention_chat_model` — a test that drives the whole graph (`get_pumpkin_response` with a real, uncompiled-away `self.graph`) needs every one of those four patched to the same mock to see every event.
