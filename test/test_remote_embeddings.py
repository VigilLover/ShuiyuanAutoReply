...
def make():
    client = Mock()
    async_client = Mock()
    settings = dict(api_key='fake', base_url='http://invalid', timeout=1, concurrency=2,
                    dims=2, model='fake', batch_size=2, attempts=1)
    return OpenAIEmbeddings(settings, client=client, async_client=async_client)


def test_order_and_dimensions():
    model = make()
    result = SimpleNamespace(data=[SimpleNamespace(index=1, embedding=[1., 0.]), SimpleNamespace(index=0, embedding=[0., 1.])])
    assert model._validate(result, 2) == [[0., 1.], [1., 0.]]
