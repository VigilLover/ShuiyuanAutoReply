import asyncio
import pytest
from shuiyuan_auto_reply.application.scheduling import ReplyScheduler, BusyError
...
def test_timeout_releases_capacity():
    async def run():
        scheduler = ReplyScheduler(1, 1, .01)
...
def test_queue_recovers_uncertain_publication(tmp_path):
    async def run():
        ...
        try:
            await db.execute("INSERT INTO forum_jobs VALUES ('bot',1,'{}','sending',NULL,0)")
            await db.execute("INSERT INTO forum_jobs VALUES ('bot',2,'{}','running',NULL,0)")
            await db.commit()
        finally:
            await db.close()
        await queue.initialize()
