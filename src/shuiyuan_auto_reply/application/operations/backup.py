...
def postgres_environment():
    ...
    env = dict(os.environ)
    env.update(PGHOST=parsed.hostname or '', PGPORT=str(parsed.port or 5432),
...
def backup(destination):
    ...
    subprocess.run(['pg_dump', '-Fc', '-f', str(target / 'postgres.dump')], env=postgres_environment(), check=True)
    local = target / 'state'
    ...
    if (state / 'state.sqlite3').exists():
        with sqlite3.connect(state / 'state.sqlite3') as source, sqlite3.connect(local / 'state.sqlite3') as dest:
            source.backup(dest)
    (target / 'manifest.json').write_text(json.dumps(dict(vector_space=get_deployment().fingerprint, complete=True)))
...
def restore(source):
    ...
    if not manifest.get('complete') or manifest['vector_space'] != get_deployment().fingerprint:
        raise ValueError('Backup vector space does not match configuration')
    state = state_directory()
    if any(state.iterdir()):
        raise ValueError('Restore requires an empty state directory and an empty target database')
    subprocess.run(['pg_restore', '--exit-on-error', '--no-owner', '--no-privileges',
                    '--dbname', postgres_environment()['PGDATABASE'], str(source / 'postgres.dump')],
