...
async def memory_store(*, indexed=True, migration=False):
    ...
    kwargs = {}
    if indexed:
        kwargs['index'] = dict(dims=config.section('embedding')['dims'], embed=get_embeddings(), fields=['content'])
    url = database_url('memory_url', migration=migration).replace('postgresql+psycopg://', 'postgresql://')
    async with AsyncPostgresStore.from_conn_string(url, **kwargs) as store:
        yield store


async def migrate_database():
    from shuiyuan_auto_reply.infrastructure.retrieval.postgres import engine_for, initialize_vector_schema
        ...
        if csv_path:
            ...
            with Path(csv_path).open(encoding='utf-8-sig', newline='') as source:
                for row in csv.DictReader(source):
                    ...
                    if not raw.strip() or settings.contains_auto_reply_tag(raw):
                        continue
                    content = re.sub(r'<div data-signature>.*?</div>', '', raw, flags=re.DOTALL).strip()
                    target.write(json.dumps(dict(persona_id=persona, text=content), ensure_ascii=False) + '\n')
                    count += 1
        else:
            ...
            async with AsyncGraphDatabase.driver(config['url'], auth=auth) as driver:
                async with driver.session(default_access_mode='READ') as session:
                    result = await session.run('MATCH (n:Sentence) WHERE ($persona IS NULL OR n.userid=$persona) RETURN n.userid AS persona_id,n.text AS text', persona=persona)
                    async for row in result:
                        if not row['persona_id']:
                            raise ValueError('Legacy corpus has no persona_id; repair source ownership first')
        ...
        with Path(output).open('x', encoding='utf-8') as target:
            ...
            while True:
                ...
                if not items:
                    break
                for item in items:
                    target.write(json.dumps(dict(namespace=list(item.namespace), key=item.key, value=item.value,
                        source_created_at=str(item.created_at), source_updated_at=str(item.updated_at)), ensure_ascii=False) + '\n')
                    count += 1
                offset += len(items)
    return count
...
async def import_data(kind, source, *, dry_run=False):
    from shuiyuan_auto_reply.infrastructure.retrieval.postgres import PostgresStyleRetriever, check_vector_space, engine_for
    source = Path(source)
    ...
    identity = dict(source=digest(source), fingerprint=get_deployment().fingerprint, kind=kind)
    ...
    try:
            ...
            with source.open(encoding='utf-8') as lines:
                for number, line in enumerate(lines, 1):
                    ...
                    try:
                        ...
                        if kind == 'corpus':
                            if not record.get('persona_id') or not record.get('text', '').strip():
                                raise ValueError('persona_id and nonempty text required')
                            content = record['text']
                            if settings.contains_auto_reply_tag(content):
                                content = ''
                            content = re.sub(r'<div data-signature>.*?</div>', '', content, flags=re.DOTALL).strip()
                            if not dry_run and content:
                                await retriever.store(record['persona_id'], content)
                        else:
                            if not isinstance(record.get('namespace'), list) or not record['namespace'] or not all(isinstance(x, str) for x in record['namespace']) or not record.get('key') or not isinstance(record.get('value'), dict):
                                raise ValueError('Invalid memory namespace/key/value')
                            if not dry_run:
                                await store.aput(tuple(record['namespace']), record['key'], record['value'])
                                from shuiyuan_auto_reply.database.postgres_memory_mgr import AsyncPostgresMemoryDatabaseManager
                                manager = AsyncPostgresMemoryDatabaseManager()
                                try:
                                    await manager.touch_mention_memory_key(record['namespace'][-1])
                                finally:
                                    await manager.close()
                        count += 1
                        if not dry_run:
        ...
        await engine.dispose()
    if errors:
        raise ValueError(f'{len(errors)} records failed; inspect failure report (dry-run does not write reports): {errors[:10]}')
    return count
