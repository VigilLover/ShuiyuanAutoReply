...
class MentionMemoryModel:
    ...
    def __init__(
        self, embedding: Embeddings, memory_settings: MemorySettings | None = None
    ):
        self.embedding = embedding
        self.postgres: Optional[AsyncPostgresMemoryDatabaseManager] = None

        current = memory_settings or MemorySettings()
        self.search_limit = current.search_limit
        self.max_context_chars = current.max_context_chars
        self.strict = current.strict
    ...
    @staticmethod
    def memory_key(user_id: int | str) -> str:
        if user_id is None:
