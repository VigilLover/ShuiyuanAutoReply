...
def parser():
    ...
    doctor.add_argument('--probe-embedding', action='store_true', help='Explicit paid embedding request')
    ...
    cookie.add_argument('--trust-pickle', action='store_true', required=True,
...
async def run(args, config):
    from shuiyuan_auto_reply.application.operations import migration
    if args.command == 'config':
        ...
        from shuiyuan_auto_reply.infrastructure.retrieval.postgres import engine_for, check_vector_space
        ...
        if args.action == 'import':
            count = await migration.import_data(args.command, args.path, dry_run=args.dry_run)
        elif args.command == 'corpus':
            count = await migration.export_corpus(args.path, csv_path=args.csv, persona=args.persona)
        else:
        ...
        values = {str(k): str(v.value if hasattr(v, 'value') else v) for k, v in cookies.items()}
        fd = os.open(args.destination, os.O_WRONLY | os.O_CREAT | os.O_EXCL, 0o600)
        with os.fdopen(fd, 'w') as target:
            json.dump({'version': 1, 'domain': 'shuiyuan.sjtu.edu.cn', 'cookies': values}, target)
    else:
        from shuiyuan_auto_reply.application.operations.backup import backup, restore
