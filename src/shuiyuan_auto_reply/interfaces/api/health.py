...
async def runtime_health(store):
    ...
    try:
        ...
        try:
            tables = await (await db.execute("SELECT name FROM sqlite_master WHERE name='forum_cursor'")).fetchone()
            if tables:
                row = await (await db.execute('SELECT last_poll FROM forum_cursor WHERE username=?', (get_deployment().section('forum')['bot_username'],))).fetchone()
                result['last_poll'] = row[0] if row else None
                result['forum'] = 'ok' if row and time.time() - row[0] < 90 else 'stale'
                rows = await (await db.execute('SELECT status,count(*) FROM forum_jobs GROUP BY status')).fetchall()
