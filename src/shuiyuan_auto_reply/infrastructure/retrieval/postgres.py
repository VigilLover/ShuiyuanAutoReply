...
def database_url(kind='url', *, migration=False):
    ...
    url = (config['migration_url'] if migration else '') or config.get(kind) or config['url']
    if not url:
...
def engine_for(kind='url', *, migration=False):
    return create_async_engine(database_url(kind, migration=migration), pool_size=1,
                               max_overflow=0, pool_pre_ping=True)


async def check_vector_space(connection):
    result = await connection.execute(text('SELECT fingerprint FROM deployment_vector_space WHERE singleton=1'))
    if result.scalar_one_or_none() != get_deployment().fingerprint:
        raise RuntimeError('Vector space mismatch: re-embed into a separate database before startup')
...
async def initialize_vector_schema(connection):
    ...
    await connection.execute(text('CREATE TABLE IF NOT EXISTS deployment_vector_space (singleton integer PRIMARY KEY CHECK(singleton=1), fingerprint text NOT NULL)'))
    await connection.execute(text('INSERT INTO deployment_vector_space VALUES (1,:fp) ON CONFLICT DO NOTHING'), {'fp': config.fingerprint})
    await check_vector_space(connection)
    await connection.execute(text(f'''CREATE TABLE IF NOT EXISTS style_sentences (
    ...
    await connection.execute(text('CREATE INDEX IF NOT EXISTS style_persona ON style_sentences(persona_id)'))
...
class PostgresStyleRetriever:
    ...
    async def search(self, persona_id, query, limit):
        ...
        async with self.engine.connect() as connection:
            await check_vector_space(connection)
            rows = await connection.execute(text('''SELECT text,
                ...
                LIMIT :limit'''), dict(vector=json.dumps(vector), persona=persona_id, limit=limit))
    ...
    async def store(self, persona_id, content):
        ...
        async with self.engine.begin() as connection:
            await check_vector_space(connection)
            await connection.execute(text('''INSERT INTO style_sentences
                ...
                dict(persona=persona_id, hash=hashlib.sha256(content.encode()).hexdigest(),
                     content=content, vector=json.dumps(vector), version=get_deployment().fingerprint))
