...
class SQLiteExecutionObserver:
    ...
    async def start(self, request: ReplyRequest) -> str:
        ...
        self.run_id = await self.store.create_run(
            request.request_id,
            conversation.id,
        ...
        await self.store.append_event(
            self.run_id, "run.started", {"channel": request.conversation.channel.value}
        )
        return self.run_id
    ...
    async def finish(
        ...
        if error is not None:
            await self.store.append_event(
                self.run_id, "run.failed", {"error": str(error)[:1000]}
            )
            await self.store.finish_run(
                self.run_id, status="failed", error=str(error)[:2000]
            )
            if self.conversation_id:
                await self.store.append_message(
                    self.conversation_id,
                    "system",
                    f"请求失败：{str(error)[:500]}",
                    run_id=self.run_id,
                    status="failed",
                )
            return
        await self.store.append_event(
            self.run_id,
            "run.completed",
            {"attachments": len(result.attachments) if result else 0},
        )
