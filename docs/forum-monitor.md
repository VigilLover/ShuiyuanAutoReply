...
任务状态为 `queued → running → publishing → completed`，异常状态包括 `failed`、`interrupted` 和 `needs_review`。只有论坛明确确认发布成功才完成；整轮无法生成有效正文时仅发布 `抱歉，发生了未知错误`，即使兜底文案发布成功也保留原任务的 `failed` 状态。发送结果不确定时保留原有队列的人工核实语义，不自动重发。
